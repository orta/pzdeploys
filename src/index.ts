...
import { Deployment } from '@vercel/sdk/esm/models/getaliasop';
...
function updateTrayMenu(renderDeploys: { service: Service, deploy: RenderDeploy["deploy"] }[] = [], vercelDeploys: { name: string, id: string, inspectorUrl: string, creator: {username: string}, meta: { githubCommitMessage: string  } }[] = []) {
  if (!tray) return;
